...
class PipelineCoordinator::Impl {
 public:
  ...
  bool Initialize() {
    ...
    drm_allocator_ = std::make_unique<neuro::hal::DRMAllocator>();
    ...
    if (cameras_.empty()) {
      ...
      rga_cfg.dst_width = config_.model_width;
      rga_cfg.dst_height = config_.model_height;
      rga_ = std::make_unique<neuro::hal::RGAProcessor>(rga_cfg);
      if (!rga_->Initialize()) {
    ...
    // RKNN engine
    neuro::inference::RKNNEngine::Config rknn_cfg;
    rknn_cfg.model_path = config_.model_path;
    rknn_cfg.core_mask = config_.npu_core_mask;
    engine_ = std::make_unique<neuro::inference::RKNNEngine>(rknn_cfg);
    ...
    // YOLO postprocessor
    neuro::inference::YOLOPostProcessor::Config yolo_cfg;
    ...
    postprocessor_ = std::make_unique<neuro::inference::YOLOPostProcessor>(yolo_cfg);

    // Memory pool for inference buffers (model_w * model_h * 3 bytes)
    size_t buf_size = config_.model_width * config_.model_height * 3;
    inference_pool_ = std::make_unique<neuro::core::MemoryPool>(
        buf_size, neuro::core::kDefaultBufferCount);
    LOG_INFO("Pipeline", "Memory pool: %zu x %zu bytes", neuro::core::kDefaultBufferCount, buf_size);

    // NPU scheduler (multi-camera mode with all cores enabled)
    if (!config_.cameras.empty() && config_.npu_core_mask == 7) {
      npu_scheduler_ = std::make_unique<neuro::inference::NPUScheduler>(
    ...
    if (config_.recording.enabled) {
      neuro::pipeline::VideoRecorder::Config rec_cfg;
      ...
      video_recorder_ = std::make_unique<neuro::pipeline::VideoRecorder>(rec_cfg);
    ...
    if (config_.enable_temporal_tracker) {
      neuro::pipeline::TemporalTracker::Config tt_cfg;
      temporal_tracker_ = std::make_unique<neuro::pipeline::TemporalTracker>(tt_cfg);
      LOG_INFO("Pipeline", "Temporal tracker enabled");
    }

    // v2: Adaptive FPS controller
    if (config_.enable_adaptive_fps) {
      neuro::pipeline::AdaptiveFPSController::Config afps_cfg;
      afps_cfg.min_fps = 5;
      afps_cfg.max_fps = config_.fps;
      adaptive_fps_ = std::make_unique<neuro::pipeline::AdaptiveFPSController>(afps_cfg);
    ...
    if (config_.enable_multi_model && !config_.models.empty()) {
      model_mgr_ = std::make_unique<neuro::inference::MultiModelManager>(
          config_.models.size());
      for (const auto& m : config_.models) {
        ...
        if (slot) {
          if (m.postprocessor == "yolov8") {
            neuro::inference::YOLOv8PostProcessor::Config v8_cfg;
            ...
            slot->postprocessor =
                std::make_unique<neuro::inference::YOLOv8PostProcessor>(v8_cfg);
          } else {
            neuro::inference::YOLOPostProcessor::Config v5_cfg;
            ...
            slot->postprocessor =
                std::make_unique<neuro::inference::YOLOPostProcessor>(v5_cfg);
          }
  ...
  void Run(std::atomic<bool>& running, double& avg_latency,
    ...
    struct PoolGuard {
      neuro::core::MemoryPool* pool;
      void* ptr;
      PoolGuard(neuro::core::MemoryPool* p) : pool(p), ptr(p ? p->Allocate() : nullptr) {}
    ...
    while (running.load()) {
      ...
      // 1. Acquire frame
      std::shared_ptr<neuro::core::Buffer> frame;
      std::shared_ptr<neuro::core::Buffer> processed;
      ...
      std::vector<neuro::core::DetectionBox> detections;
      {
        ...
        if (model_mgr_) {
          ...
          if (active && active->engine) {
            ...
            neuro::pipeline::EdgeMetrics::Instance().RecordInferenceLatencyMs(infer_ms);
      ...
      if (temporal_tracker_) {
        ...
        for (const auto& [tid, btype] : behaviors) {
          if (btype != neuro::pipeline::BehaviorType::kNone && grpc_client_ && grpc_client_->IsConnected()) {
      ...
      std::vector<neuro::core::DetectionBox> novel_detections;
      ...
      if (!novel_detections.empty()) {
        neuro::pipeline::EdgeMetrics::Instance().IncrementDetectionsTotal(novel_detections.size());
      ...
      frame_count++;
      fps_frames++;
      neuro::pipeline::EdgeMetrics::Instance().IncrementFramesProcessed();

      // Update FPS every second
      auto fps_elapsed = std::chrono::duration<double>(t1 - fps_start).count();
      if (fps_elapsed >= neuro::core::kFPSUpdateIntervalSec) {
        measured_fps = static_cast<uint32_t>(fps_frames / fps_elapsed);
        neuro::pipeline::EdgeMetrics::Instance().SetFPS(static_cast<double>(measured_fps));
        fps_frames = 0;
        fps_start = t1;
      }

      auto health_elapsed = std::chrono::duration<double>(t1 - last_health).count();
      if (health_elapsed >= neuro::core::kHealthUpdateIntervalSec && grpc_client_ && grpc_client_->IsConnected()) {
        ...
        auto& metrics = neuro::pipeline::EdgeMetrics::Instance();
 ...
 private:
  ...
  neuro::pipeline::DetectionCache detection_cache_;
  std::unique_ptr<neuro::core::MemoryPool> inference_pool_;
  std::unique_ptr<neuro::inference::NPUScheduler> npu_scheduler_;
  std::unique_ptr<neuro::pipeline::VideoRecorder> video_recorder_;
